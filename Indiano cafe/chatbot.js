...
(function () {
    ...
    const MENU = {
        healthyCombos: [
            { name: "Green Goddess Bowl", price: 249, desc: "Quinoa, avocado, greens, hummus & lemon dressing", cal: 380 },
            { name: "Protein Power Plate", price: 279, desc: "Grilled paneer, sprouts, boiled eggs & multigrain toast", cal: 420 },
            { name: "Detox Delight", price: 199, desc: "Fresh juice, oats bowl & mixed fruits", cal: 290 },
            { name: "Fit & Fresh Combo", price: 229, desc: "Grilled sandwich, green smoothie & salad", cal: 350 },
        ],
        todaysSpecial: [
            { name: "Masala Dosa Feast", price: 179, desc: "Crispy masala dosa with sambar, chutney & filter coffee", cal: 450 },
            { name: "Royal Thali", price: 349, desc: "Dal, paneer, 3 rotis, rice, raita, salad & dessert", cal: 680 },
            { name: "Chef's Pasta Supreme", price: 269, desc: "Creamy alfredo pasta with garlic bread & cold coffee", cal: 520 },
        ],
        budgetMeals: [
            { name: "Student Saver", price: 99, desc: "Veg sandwich + chai", cal: 310 },
            { name: "Quick Bite Combo", price: 129, desc: "Samosa (2pcs) + masala chai + cookies", cal: 380 },
            { name: "Light Lunch", price: 149, desc: "Mini thali: dal, rice, roti & pickle", cal: 400 },
        ],
        drinks: [
    ...
    const CONFUSION_WORDS = [
        "confused", "confuse", "don't know", "not sure", "maybe",
        "i think", "hmm", "what should", "suggest me", "help me",
        "can't decide", "which one", "recommend", "idk", "no idea",
    ];
    ...
    function extractInterests(text) {
        ...
        const map = {
            healthy: ["healthy", "fit", "diet", "low cal", "salad", "detox", "protein"],
            budget: ["budget", "cheap", "affordable", "student", "saver", "economical"],
    ...
    function getSuggestions() {
        ...
        if (interests.has("healthy")) return ["Green Goddess Bowl", "Protein Plate", "Detox Menu"];
        if (interests.has("budget")) return ["Student Saver", "Value Combo", "Under ₹150"];
    ...
    function updateSuggestionButtons() {
        ...
        if (!container) return;
        container.innerHTML = "";
        getSuggestions().slice(0, 4).forEach(text => {
    ...
    function init() {
        ...
        setTimeout(() => {
            ...
            addMessage(`${greeting}! Welcome to **Indiano Cafe**. How can I help you today?`, "bot");
            updateSuggestionButtons();
        }, 1000);
    }
